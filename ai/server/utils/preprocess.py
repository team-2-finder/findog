...
def get_crops(paths):
    ...
    for idx, (dog_img, dog_mask) in enumerate(zip(dog_list, dog_masks)):
        ...
        masked_img = np_img.copy()  # Initialize masked image array as a copy of original image

        for c in range(np_img.shape[0]):
            masked_img[c] = np.where(np_mask > 0.01, np_img[c], 0)  # Apply the mask to the corresponding channel
